...
const getAll = async (req, res) => {
    ...
    const validatedNotifications = notifications.map((notification) => {
        const {type, checked, receiver, userRef, postRef = null, collectionRef = null, commentRef = null, subCommentRef = null} = notification
        ...
        if (type === notificationTypes.addCommentToPost && commentRef && postRef) {
            ...
            const {_id: postId, image: {url}} = postRef

            return {checked, type, user, _id, post: {_id: postId, image: url}, comment: {text, _id: commentId}}
        }
        ...
        if (type === notificationTypes.addReplyToComment && subCommentRef && postRef) {
            ...
            return {checked, type, user, _id, post: {_id: postId, image: url}, comment: {text, _id: subCommentId}}
        }
