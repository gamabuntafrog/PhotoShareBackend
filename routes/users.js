...
router.get(
  '/:id/allowedToViewCollections',
  ctrlWrapper(auth),
  ctrlWrapper(ctrl.getAllowedToViewCollections)
)

router.get('/search', ctrlWrapper(ctrl.getUsersByUsername))

router.get(
  '/search/forAddInCollection',
  ctrlWrapper(auth),
  ctrlWrapper(ctrl.getUsersForAddInCollection)
)
...
router.post(
  ...
  validateObjectId(),
  ctrlWrapper(auth),
  ctrlWrapper(ctrl.addToSubscribes)
)
...
router.delete(
  ...
  validateObjectId(),
  ctrlWrapper(auth),
  ctrlWrapper(ctrl.deleteFromSubscribes)
)
